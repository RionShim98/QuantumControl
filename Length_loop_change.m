...
L2=L1;
L3=300;
Ln=[0.5 5 10 50];% Vector
leg={'LIGO [Eq.~(41)]','SQL',...
    '$L_\textrm{loop}=0.5$ m',...
    '$L_\textrm{loop}=5$ m',...
    '$L_\textrm{loop}=10$ m',...
    '$L_\textrm{loop}=50$ m'};
los1=c*Tlos1/L1;% a_1 loss
los2=c*Tlos2/L2;% a_2 loss
...
Atot=zeros(24,24,length(Ln));
col={'b:','g--','c:','r-.'};
for i=1:length(Ln)

g24=sqrt(c*gam/Ln(i));
g34=sqrt(c*kap1/Ln(i));
los4=c*Tlos4/Ln(i);% a_4 (loop) loss
fprintf('los4 = %d\n',los4)
...
C1m=[0 0 0 sqrt(IFO) 0 0 0 0 0 0 0 0];
